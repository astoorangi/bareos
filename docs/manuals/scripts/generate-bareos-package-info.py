...
class Dist:
    def __init__(self, obsname):
        ...
        self.version = (
            self.version.replace("_", "").replace("Leap", "").replace("SP", "sp")
        )
        if self.dist == "Debian":
...
class LatexTable(BaseTable):
    def getIndexes(self):
        ...
        for i in self.columns:
            result += "\\index[general]{{Platform!{0}!{1}}}".format(
                Dist(i).dist, Dist(i).version
            )
        return result
...
class PackageDistReleaseData:
    def __init__(self):
        ...
        self.package_exclude_pattern = re.compile(
            "php5-zendframework2|php-ZendFramework2.*|mhvtl.*|mingw.*|.*-dbg|.*-debug.*|.*-doc|.*-devel|.*-dev"
        )
    ...
    def generate(self, inputdata):
        ...
        for release in list(inputdata.keys()):
...
def generate_overview_table(packageDistReleaseData, dist_include, latex):
    ...
    distributions = sorted(
        get_matches(packageDistReleaseData.distributions, dist_include)
    )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Generate information about packages in distributions."
    )
    ...
    parser.add_argument(
        "--sphinx", action="store_true", help="Create only RST files for Sphinx."
    )
    parser.add_argument(
        "filename", nargs="+", help="JSON files containing package information"
    )
    ...
    create_file(
        filenametemplate.format("opensuse"),
        data,
        "openSUSE_13..|openSUSE_Leap_.*",
        args.latex,
    )
    create_file(
        filenametemplate.format("suse"),
        data,
        "SLE_10_SP4|SLE_11_SP4|SLE_12_.*|SLE_15_.*",
        args.latex,
    )
